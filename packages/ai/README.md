...
#### 3. Model Generation (`scripts/model-generation/`, `scripts/generate-image-models.ts`)

- Declare the provider in `scripts/model-generation/providers/<id>.ts` with `defineProvider()`: a source (usually `modelsDev()`), static models upstream lacks, and `rules` for provider-specific corrections; register it in `providers/index.ts`. See `scripts/model-generation/README.md`
- Hydration groups the ignored `src/providers/data/<id>.json` values by API, while stable `src/providers/<id>.models.ts` wrappers derive exact model/API types directly from those JSON keys
